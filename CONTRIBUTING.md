# Contributing to Codebase Intelligence
...
- `engine/` — the engine (pure TS, zero-network). Tests: `cd engine && node scripts/test.ts && node scripts/eval.ts`.
...
npm test             # typecheck (tsc --noEmit, strict) + unit + integration + retrieval eval
